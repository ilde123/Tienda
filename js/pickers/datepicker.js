...
gj.datepicker.config = {
	base: {
		...
		icons: {
			 ...
			 */
			rightIcon: '<i class="gj-icon pt-1">event</i>',
...
gj.datepicker.methods = {
	...
	renderMonth: function ($datepicker, $calendar, data) {
		...
		for (i = 1; i <= total; i++) {
			...
			if (date.getDay() === 6 || date.getDay() === 0) { // Marcar en rojo los fines de semana
				$cell.addClass('text-danger font-weight-bolder');
			}
/*
			if (gj.datepicker.methods.isFestive(date)) {
...
*/
	...
	isFestive: function (date, data) {
		var result = false,
			...
			weekDay = date.getDay(),
			weekOfYear = gj.datepicker.methods.getWeekNumber(date),
			holidays = data.holidays;
		if (holidays) {
			for (i = 0; i < holidays.length; i++) {
				if (holidays[i].date.getDate() === day && holidays[i].date.getMonth() === month) { // If it is a date
					result = true;
					break;
				} else if (holidays[i].weekEnd === weekDay) { // Sunday is 0, Monday is 1, etc
					result = true;
					break;
				} else if (holidays[i].week === weekOfYear) { //ISO 8601: First week contains first thursday.
					//If week number is the same as the date's week number, then it must be in the range
					if (holidays[i].start === 'week') {
						holidays[i].start = new Date(year, 0, 1);
					}
					if (holidays[i].end === 'week') {
						holidays[i].end = new Date(year, 11, 31);
					}
					if ((holidays[i].start.valueOf() <= date.valueOf()) && (holidays[i].end.valueOf() >= date.valueOf())) { //The date is in the date range
						result = true;
						break;
					}
				}
